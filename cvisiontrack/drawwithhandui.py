...
def draw(x, y):  # Modified to take x, y directly
...
def start_draw(x, y):  # Modified to take x, y directly
    global drawing, start_point
    drawing = True
    start_point = (x, y)


def stop_draw():  # Modified to not take event
    global drawing, start_point
    drawing = False
    start_point = None
...
def update_frame():
    ...
    and draws hand landmarks based on hand position.
    """
    ret, frame = cam.read()
    if not ret:
    ...
    if results.multi_hand_landmarks:
        for hand_landmarks in results.multi_hand_landmarks:
            ...
            #  Control drawing based on a gesture (e.g., pinching)
            thumb_tip = hand_landmarks.landmark[mp_hands.HandLandmark.THUMB_TIP]
            distance = (
                (index_tip.x - thumb_tip.x) ** 2 + (index_tip.y - thumb_tip.y) ** 2
            ) ** 0.5

            if distance < 0.1:  # Adjust this threshold as needed (Pinch)
                if not drawing:
                    start_draw(index_x, index_y)  # Start drawing
                draw(index_x, index_y)  # Continue drawing
            else:
                stop_draw()
